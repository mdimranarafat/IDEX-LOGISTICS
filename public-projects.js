...
// ---- Firestore live query: published projects ----
// NOTE: no orderBy('completionDate') here — Firestore silently excludes
// documents where the ordered field is null/missing. We sort client-side instead.
const q = query(
...
onSnapshot(q, (snap) => {
  ...
  // Sort newest → oldest by completionDate; projects without a date go last.
  const timeOf = p => (p.completionDate && p.completionDate.toDate ? p.completionDate.toDate().getTime() : null);
  projects.sort((a, b) => {
    const ta = timeOf(a);
    const tb = timeOf(b);
